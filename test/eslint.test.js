...
test('javascript', async () => {
	const badFixture = await readFile('./test/fixtures/eslint/bad/javascript.js');
	...
	const goodFixture = await readFile('./test/fixtures/eslint/good/javascript.js');
...
test('json', async () => {
	const badFixture = await readFile('./test/fixtures/eslint/bad/json.json');
	...
	const goodFixture = await readFile('./test/fixtures/eslint/good/json.json');
...
test('jsonc', async () => {
	const badFixture = await readFile('./test/fixtures/eslint/bad/jsonc.jsonc');
	const badFixtureErrors = await runEslint(badFixture.toString(), eslintConfig, 'jsonc');
	assert.ok(hasRule(badFixtureErrors, 'json/no-duplicate-keys'));

	const goodFixture = await readFile('./test/fixtures/eslint/good/jsonc.jsonc');
	const goodFixtureErrors = await runEslint(goodFixture.toString(), eslintConfig, 'jsonc');
	assert.ok(!hasRule(goodFixtureErrors, 'json/no-duplicate-keys'));
});

test('markdown', async () => {
	const badFixture = await readFile('./test/fixtures/eslint/bad/markdown.md');
	...
	const goodFixture = await readFile('./test/fixtures/eslint/good/markdown.md');
